    ...
    renderPagination(dataPagination){
        ...
        for(let i = 1; i <= dataPagination.countPage; i++){
            ...
            paginationItem.addEventListener('click', (e)=>{
                ...
                // на элемент, на который я кликнул, а является объектом catalog
...
//     // let product = new Products(data1.id, data1.active, data1.name, data1.price, data1.photo, data1.sku, data1.description);

//     // let product2 = new Products(data2.id, data2.active, data2.name, data2.price, data2.photo, data2.sku, data2.description);
...
//     data.products.forEach((item)=>{
//         let product = new Products(item.id, item.active, item.name, item.price, item.photo, item.sku, item.description);
